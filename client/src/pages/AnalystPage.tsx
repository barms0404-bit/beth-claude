...
const analystDatabase: Record<string, AnalystData> = {
  ...
  "beth": {
    ...
    bio: "Beth is the Chief of Staff and central nervous system of the entire 76-professional research operation. She reports directly to both Brian Armstrong (Managing Partner, Portfolio Management) and Samira Arikat (Managing Partner, Financial Planning). MBA, Columbia Business School. BA Economics, Georgetown University. 18 years on Wall Street: JPMorgan Asset Management, Bridgewater Associates, now Armstrong Arikat. She orchestrates all 36 senior analysts across 9 divisions, manages the 5-layer compliance pipeline, resolves conflicts between analysts, and synthesizes all inputs into actionable intelligence. Her fundamental obligation: protect the firm's time and capital with intellectual honesty.",
  ...
  "samira-arikat": {
    name: "Samira Arikat",
    role: "Managing Partner, Financial Planning",
    pod: "Executive",
    model: "N/A",
    temp: "N/A",
    trackRecord: "Co-Founder",
    hitRate: "N/A",
    weight: "N/A (Managing Partner)",
    bio: "Samira Arikat is Managing Partner and Co-Founder of Armstrong Arikat Private Wealth Group, leading the Financial Planning division. She oversees estate planning, tax optimization, retirement strategies, insurance, and wealth transfer for the firm's ultra-high-net-worth clients. CFP, ChFC, CLU certified. BS Business Administration from San Francisco State University. 20+ years in wealth management spanning Morgan Stanley Private Wealth, Goldman Sachs Private Client Services, and now Armstrong Arikat.",
    methodology: "Samira's financial planning framework: (1) Comprehensive wealth assessment — total balance sheet including illiquid assets, business interests, and real estate, (2) Tax-optimized structure design — trusts, LLCs, family limited partnerships, (3) Retirement income modeling — Monte Carlo simulations across market scenarios, (4) Estate and wealth transfer — generation-skipping trusts, charitable vehicles, dynasty trusts, (5) Risk management — umbrella liability, key person insurance, business succession planning.",
    currentView: "Client wealth preservation is paramount in this elevated market environment. With S&P at all-time highs, now is the time to review concentrated positions for tax-loss harvesting opportunities in other areas, consider Roth conversions while tax rates are historically favorable, and ensure estate documents reflect current asset values. Working with Beth to ensure investment recommendations align with each client's overall financial plan and tax situation.",
    recommendations: []
  },
  "omar-hamze": {
    name: "Omar Hamze",
    role: "Advisor in Training — Assistant to Beth",
    pod: "Executive",
    model: "claude-opus-4-7",
    temp: "0.2",
    trackRecord: "New — In Training",
    hitRate: "N/A (in training)",
    weight: "N/A (not a decision maker)",
    bio: "Omar Hamze is an Advisor in Training at Armstrong Arikat Private Wealth Group, serving as Beth's executive assistant. He supports Beth with research coordination, data compilation, report formatting, and operational tasks. Series 65 and Series 7 licensed. BS Finance from Grand Canyon University. Omar is learning the firm's investment process from the ground up and is not authorized to make investment decisions or client recommendations independently.",
    methodology: "Omar's role is operational support: (1) Compile daily market data for Beth's morning briefing, (2) Format and distribute the 3x daily reports, (3) Track specialist output completion and flag delays, (4) Maintain the recommendation database and performance tracking, (5) Coordinate meeting schedules between Brian, Samira, and Beth, (6) Research tasks as assigned by Beth — data gathering, not analysis.",
    currentView: "Supporting Beth with today's operational tasks. All 36 senior analysts have submitted morning dispatches. Performance tracking database updated. Report distribution confirmed for brian@cccballers.com. Earnings calendar flagged for NVDA (Wednesday) and DELL/CRM (Thursday). Standing by for additional assignments.",
    recommendations: []
  },
};
  ...
  return (
    <div className="min-h-screen bg-[#000000] relative">
      ...
      <main className="relative z-10 px-4 py-6 max-w-[1200px] mx-auto space-y-8">
        ...
        {/* LIVE AI RESEARCH — PRIMARY CONTENT (moved to top) */}
        <section className="bg-[#0F0F0F] border border-[#1F1A0F] rounded-lg p-6 border-l-4 border-l-[#4ADE80]">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Zap className="w-5 h-5 text-[#4ADE80]" />
              <h2 className="text-[#4ADE80] text-xl" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Detailed Research & Recommendations</h2>
            </div>
            <button onClick={() => refetch()} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#4ADE80]/10 border border-[#4ADE80]/30 hover:bg-[#4ADE80]/20 transition-colors">
              <RefreshCw className={`w-3.5 h-3.5 text-[#4ADE80] ${researchLoading ? "animate-spin" : ""}`} />
              <span className="text-[#4ADE80] text-[10px] uppercase tracking-[1px] font-semibold">Generate Fresh Research</span>
            </button>
          </div>
          <p className="text-[#8A7548] text-xs mb-4">This is {analyst.name}'s live detailed research dispatch — generated using real-time market data from Polygon.io, FRED, Alpha Vantage, Perplexity, and CoinGecko. Click "Generate Fresh Research" for the latest analysis.</p>
          {researchLoading ? (
            <div className="flex items-center gap-3 py-8">
              <RefreshCw className="w-5 h-5 text-[#C9A961] animate-spin" />
              <p className="text-[#F5E6C8] text-sm">{analyst.name} is analyzing current market conditions and generating detailed research...</p>
            </div>
          ) : liveResearch?.research ? (
            <div className="prose prose-sm prose-invert max-w-none">
              <div className="text-[#F5E6C8] text-sm leading-[1.8] whitespace-pre-wrap"><Streamdown>{liveResearch.research}</Streamdown></div>
              <div className="mt-4 pt-3 border-t border-[#1F1A0F] flex items-center justify-between">
                <p className="text-[#8A7548] text-[10px]">Generated: {new Date(liveResearch.timestamp).toLocaleString()} | Model: {(liveResearch as any).model || 'Claude'} | Data: Polygon + FRED + Alpha Vantage + Perplexity</p>
                <p className="text-[#4ADE80] text-[10px]">Recommendations auto-logged to performance tracker</p>
              </div>
            </div>
          ) : (
            <div className="py-8 text-center">
              <Bot className="w-8 h-8 text-[#C9A961] mx-auto mb-3" />
              <p className="text-[#F5E6C8] text-sm mb-2">Click "Generate Fresh Research" to get {analyst.name}'s detailed analysis</p>
              <p className="text-[#8A7548] text-xs">Research includes: Current View, Top Recommendation with WHY/HOW/RISKS, Secondary Picks, and specific price targets based on live market data.</p>
            </div>
          )}
        </section>

        {/* Current Market View (static baseline) */}
        <section className="bg-[#0F0F0F] border border-[#1F1A0F] rounded-lg p-5 border-l-4 border-l-[#C9A961]">
          <h2 className="text-[#C9A961] text-lg mb-3" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{analyst.name}'s Standing View</h2>
        ...
        {/* Old Live AI Research section removed — moved to top of page */}
